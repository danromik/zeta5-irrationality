/-
Zeta5/Sec6/LogCosOff.lean  —  (A.1) off the interval, normalised (`|x| > 1`).
-/
...
/-- (A.1) off the interval, normalised:
...
Proof.  With the real root `ζ = x + sign(x)√(x²−1)`, `|ζ| = |x| + √(x²−1) > 1`,
`|x − cos θ| = |e^{iθ} − ζ|²/(2|ζ|)`, so by Mathlib's `circleAverage_log_norm_sub_const₂`
the integral over `[0,2π]` is `2π(2 log|ζ| − log 2 − log|ζ|)`; the integral over `[0,π]` is
half of it, since `cos(2π − θ) = cos θ`. -/
