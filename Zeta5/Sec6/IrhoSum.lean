/-
Zeta5/Sec6/IrhoSum.lean  —  (A.2) as a double sum over pairs of components.
-/
...
Proof.  By induction on `n`, `∑_{i,j<n} c_i c_j f(max i j) = ∑_{m<n} (S_{m+1}² − S_m²) f(m)`
with `S_m = ∑_{i<m} c_i` (`Irho_double_sum_aux`): the step adds the terms with `i = n` or
`j = n`, totalling `(2c_n S_n + c_n²) f(n)`. -/
