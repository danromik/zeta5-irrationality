/-
Zeta5/Sec6/Frullani.lean  —  the Frullani representation of the Cauchy kernel.
...
/-- The Frullani integrand `s ↦ e^{-sε²}(1-e^{-sd²})/s` is
integrable on `(0,∞)`.

Paper: implicit in the proof of Lemma 6.2 (p. 18).

Proof.  For `s > 0`, `0 ≤ frF ε s d ≤ d² e^{-sε²}` (from `0 ≤ 1 − e^{-x} ≤ x`), and the
majorant is integrable on `Ioi 0`; the integrand is continuous there. -/
...
/-- **Frullani's integral for the Cauchy kernel**:
...
Proof.  Mathlib's `Frullani.integral_Ioi_eq` with `f(x) = e^{-x}`, `a = ε²`, `b = ε² + d²`,
`L = 1`, `R = 0`, after rewriting `frF ε s d = s⁻¹(f(ε²s) − f((ε² + d²)s))`; integrability is
`frullani_integrable`. -/
