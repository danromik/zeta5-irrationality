/-
cert/axioms/AScan.lean — cert/final/FScan.lean re-run on branch `axioms` (commit ca05d77).  The
code of sections [1]–[6] and the planted control are copied verbatim; the only change is the
list `projAx` of project axioms whose direct users section [4] reports, which is now the single
axiom `Zeta5.Axioms.chebyshev_theta_asymptotic` (the two former axioms no longer exist, and
naming them in a double-backtick literal would not elaborate).  The Zeta5-module filter is by
module-name prefix, so the new modules Zeta5.Hermite and Zeta5.PNT are included automatically.

Run from the repository root, after `lake build`:
  lake env lean cert/axioms/AScan.lean

FScan.lean header follows.
...
elab "#c3scan" : command => do
  ...
  let projAx : Array Name := #[``Zeta5.Axioms.chebyshev_theta_asymptotic]
