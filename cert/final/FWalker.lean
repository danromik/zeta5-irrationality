/-
cert/final/FWalker.lean — the C3 walker (cert/C3Walker.lean), re-run on branch eq614 after
(6.14) was proved in Zeta5/Sec6/.  The walker code (constsOf, succs, walk, chain, #c3) is
copied verbatim from cert/C3Walker.lean; only the target list changed (the §6 modules, the 19
blueprint leaves, and the module each `eq_6_14`/`prop_6_3` constant is defined in), and two
further planted controls were added (a `sorry` hidden in an auxiliary `where` definition and
a `sorry` reached only through a `.proj`/instance path).
